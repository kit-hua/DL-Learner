 ...
 *
 */
@ComponentAnn(name = "rho refinement operator (mcts version)", shortName = "rho_mcts", version = 0.8)
public class RhoDRDownMCTS extends RhoDRDown implements Component, CustomHierarchyRefinementOperator, CustomStartRefinementOperator, ReasoningBasedRefinementOperator {

	private static Logger logger = LoggerFactory.getLogger(RhoDRDownMCTS.class);
...
//	private Map<OWLClass,Map<OWLClass,Boolean>> notABMeaningful = new TreeMap<OWLClass,Map<OWLClass,Boolean>>();
	...
	public RhoDRDownMCTS() {}

	public RhoDRDownMCTS(RhoDRDownMCTS op) {
...
//		System.out.println(refinements);
	...
	public Set<OWLClassExpression> refineSimulation(OWLClassExpression description, int maxLength) {
		// check that maxLength is valid
		if(maxLength < OWLClassExpressionUtils.getLength(description, lengthMetric)) {
			throw new Error("length has to be at least class expression length (class expression: " + description + " with length " + OWLClassExpressionUtils.getLength(description, lengthMetric) +", and max length: " + maxLength + ")");
		}
		return refineSimulation(description, maxLength, null, startClass);
	}
...
//		System.out.println("|- " + description + " " + currDomain + " " + maxLength);
		...
		// here; and whether HashSet or TreeSet should be used
		// => TreeSet because duplicates are possible
		List<Set<OWLClassExpression>> refinements = new ArrayList<>(); // = new TreeSet<>();
		...
		if(description.isOWLThing()) {
			...
			if(currDomain.isOWLThing()) {
				if(maxLength>topRefinementsLength)
					computeTopRefinements(maxLength);
				//Set<OWLClassExpression> localRefinements = (TreeSet<OWLClassExpression>) topRefinementsCumulative.get(maxLength).clone();
				//refinements.add(localRefinements);
				for (int i = 1; i <= maxLength; i++) {
					Set<OWLClassExpression> localRefinements = topRefinements.get(i);
					if (localRefinements != null) {
						refinements.add(new TreeSet<>(localRefinements));
					}
				}
				...
				//Set<OWLClassExpression> localRefinements = (TreeSet<OWLClassExpression>) topARefinementsCumulative.get(currDomain).get(maxLength).clone();
				//refinements.add(localRefinements);
				for (int i = 0; i <= maxLength; i++) {
					Set<OWLClassExpression> localRefinements = topARefinements.get(currDomain).get(i);
					if (localRefinements != null) {
						refinements.add(new TreeSet<>(localRefinements));
					}
				}
...
//			refinements.addAll(classHierarchy.getMoreSpecialConcepts(description));
			...
			Set<OWLClassExpression> localRefinements = new TreeSet<>(classHierarchy.getSubClasses(description, true));
			localRefinements.remove(df.getOWLNothing());
			refinements.add(localRefinements);
		} else if (description instanceof OWLObjectComplementOf) {
			OWLClassExpression operand = ((OWLObjectComplementOf) description).getOperand();
			Set<OWLClassExpression> localRefinements = new TreeSet<>();
			if(!operand.isAnonymous()){
				tmp = classHierarchy.getSuperClasses(operand, true);

				for(OWLClassExpression c : tmp) {
					if(!c.isOWLThing()){
						localRefinements.add(df.getOWLObjectComplementOf(c));
					}
				}
			}
			refinements.add(localRefinements);
			...
			for(OWLClassExpression child : operands) {
				...
				int length = OWLClassExpressionUtils.getLength(description, lengthMetric);
				int childLength = OWLClassExpressionUtils.getLength(child, lengthMetric);
				tmp = refineSimulation(child, maxLength - length + childLength, null, currDomain);
...
//				System.out.println("union child: " + child + " " + maxLength + " " + description.getLength() + " " + child.getLength());
			...
			if(dropDisjuncts) {
				// A1 OR A2 => {A1,A2}
				if(operands.size() == 2) {
					localRefinements.add(operands.get(0));
					localRefinements.add(operands.get(1));
				} else {
					// copy children list and remove a different element in each turn
					for(int i=0; i<operands.size(); i++) {
						List<OWLClassExpression> newChildren = new LinkedList<>(operands);
			...
			Set<OWLClassExpression> localRefinements1 = new TreeSet<>();
			Set<OWLClassExpression> localRefinements2 = new TreeSet<>();
			Set<OWLClassExpression> localRefinements3 = new TreeSet<>();
			Set<OWLClassExpression> localRefinements4 = new TreeSet<>();
			...
			// rule 1: EXISTS r.D => EXISTS r.E
			tmp = refineSimulation(filler, maxLength-lengthMetric.objectSomeValuesLength-lengthMetric.objectProperyLength, null, domain);

			for(OWLClassExpression c : tmp){
				localRefinements1.add(df.getOWLObjectSomeValuesFrom(role, c));
			}
			...
			for (OWLObjectProperty moreSpecialRole : moreSpecialRoles) {
				localRefinements2.add(df.getOWLObjectSomeValuesFrom(moreSpecialRole, filler));
			}
			...
			if(useHasValueConstructor && filler.isOWLThing()){ // && !role.isAnonymous()) {
				...
				if(frequentInds != null) {
					for(OWLIndividual ind : frequentInds) {
						OWLObjectHasValue ovr = df.getOWLObjectHasValue(role, ind);
						localRefinements4.add(ovr);
						if(useObjectValueNegation ){
							localRefinements4.add(df.getOWLObjectComplementOf(ovr));
						}
			...
			refinements.add(localRefinements1);
			refinements.add(localRefinements2);
			refinements.add(localRefinements3);
			refinements.add(localRefinements4);
			...
			// refinements.addAll(refine((OWLObjectAllValuesFrom) description, maxLength));
			OWLObjectAllValuesFrom ce = (OWLObjectAllValuesFrom) description;
			...
			tmp = refineSimulation(filler, maxLength-lengthMetric.objectAllValuesLength-lengthMetric.objectProperyLength, null, range);

			for(OWLClassExpression c : tmp) {
				localRefinements1.add(df.getOWLObjectAllValuesFrom(role, c));
			}

			// rule 2: ALL r.D => ALL r.BOTTOM if D is a most specific atomic concept
			if(!filler.isOWLNothing() && !filler.isAnonymous() && tmp.size()==0) {
				localRefinements2.add(df.getOWLObjectAllValuesFrom(role, df.getOWLNothing()));
			}
			...
			for (OWLObjectProperty subProperty : subProperties) {
				localRefinements3.add(df.getOWLObjectAllValuesFrom(subProperty, filler));
			}
			...
			if(description instanceof OWLObjectMaxCardinality) {
				// rule 1: <= x r.C =>  <= x r.D
				if(useNegation || cardinality > 0){
					tmp = refineSimulation(filler, maxLength-lengthMetric.objectCardinalityLength-lengthMetric.objectProperyLength, null, range);

					for(OWLClassExpression d : tmp) {
						localRefinements1.add(df.getOWLObjectMaxCardinality(cardinality,role,d));
					}
				}
...
//				int number = max.getNumber();
				if((useNegation && cardinality > 1) || (!useNegation && cardinality > 2)){
					localRefinements2.add(df.getOWLObjectMaxCardinality(cardinality-1,role,filler));
				}
				...
				for(OWLClassExpression d : tmp) {
					localRefinements1.add(df.getOWLObjectMinCardinality(cardinality,role,d));
				}

				// >= x r.C  =>  >= (x+1) r.C
//				int number = min.getNumber();
				if(cardinality < maxNrOfFillers.get(role)){
					localRefinements2.add(df.getOWLObjectMinCardinality(cardinality+1,role,filler));
				}
			} else if(description instanceof OWLObjectExactCardinality) {
				tmp = refineSimulation(filler, maxLength-lengthMetric.objectCardinalityLength-lengthMetric.objectProperyLength, null, range);

				for(OWLClassExpression d : tmp) {
					localRefinements1.add(df.getOWLObjectExactCardinality(cardinality,role,d));
				}

				// >= x r.C  =>  >= (x+1) r.C
//				int number = min.getNumber();
				if(cardinality < maxNrOfFillers.get(role)){
					localRefinements2.add(df.getOWLObjectExactCardinality(cardinality+1,role,filler));
				}
			}
			...
			if(dr instanceof OWLDatatypeRestriction){
				...
				if(newDatatypeRestriction != null){
					localRefinements.add(df.getOWLDataSomeValuesFrom(dp, newDatatypeRestriction));
				}
			}
			...
			if(!dp.isAnonymous()){
				Set<OWLDataProperty> subDPs = dataPropertyHierarchy.getMoreSpecialRoles(dp.asOWLDataProperty());
				for(OWLDataProperty subDP : subDPs) {
					localRefinements.add(df.getOWLDataHasValue(subDP, value));
				}
			}
		...
		if(!description.isOWLThing() && !description.isOWLNothing()
			...
			List<OWLClassExpression> operands = Lists.newArrayList(description, OWL_THING);
			Collections.sort(operands);
			OWLObjectIntersectionOf mc = new OWLObjectIntersectionOfImplExt(operands);
...
//		System.out.println(refinements);
		...
		// Remove refinement sets that are empty
		refinements.removeIf(Set::isEmpty);

		if (refinements.isEmpty()) {
			return new TreeSet<>();
		} else {
			// Choose random refinement set -> i.e. sample from the classes that we defined
			int r = rnd.nextInt(refinements.size());
			return refinements.get(r);
		}
